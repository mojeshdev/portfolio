...
export default function Home() {
  ...
  // Disable body scroll on popup open
  useEffect(() => {
    if (showPopup) {
  ...
  // Show loader first, fade out, then show home
  useEffect(() => {
    const timer1 = setTimeout(() => setFadeOut(true), 2000); // start fading after 2s
    const timer2 = setTimeout(() => setLoading(false), 2700); // remove completely
    return () => {
      clearTimeout(timer1);
      clearTimeout(timer2);
    };
  ...
  return (
    <div className="relative h-screen overflow-hidden flex flex-col lg:flex-row animate-[fadeInUp_0.8s_ease-out_forwards]">
      {/* Yellow Background */}
      ...
      <div className="mb-52">
        <div
          className="
            ...
            max-w-xl mx-auto
            lg:absolute lg:right-64 lg:top-1/2 lg:-translate-y-1/2
            mt-48 lg:mt-10
          "
        >
          <div className="text-center lg:text-left max-w-3xl mx-auto">
            <h1 className="font-sans font-extrabold text-3xl lg:text-5xl text-yellow-500 mb-4">
              -- I'm Lamu Vidyardhi.
              <span className="block text-white text-3xl lg:text-5xl lg:ml-14">
                Web Designer
              </span>
            </h1>
            <p className="text-white text-md lg:text-lg lg:ml-14 mb-8 px-2 lg:px-0">
              I'm a web designer, front-end & back-end developer focused on
